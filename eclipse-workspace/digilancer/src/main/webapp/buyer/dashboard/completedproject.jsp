   ...
   <div class="container">
    ...
    <%
		...
		int clientid = cmodel.getclientId();
	 	PreparedStatement st = cn.prepareStatement("SELECT p.projectid, p.projectname, p.category, p.description, p.min, p.max, p.clientid, p.status, p.amount, p.freelancerid, CONCAT(f.firstname, ' ', f.lastname) AS freelancer_name FROM project p LEFT JOIN freelancer f ON p.freelancerid = f.freelancerid where clientid=? and status='completed'");
   		...
   		<div class="w-100 myproject rounded p-3 mt-3">
   			<div class="d-flex flex-column align-items-center justify-content-center">
	   			...
	   			<div class="details mt-4 d-flex flex-column justify-content-center">
		   			...
		   			<a class="mr-auto ml-auto" href="viewfreelancerprofile.jsp?source=completedprojects&freelancerid=<%=rs.getInt("freelancerid") %>" ><%=rs.getString("freelancer_name") %> </a>
	   			</div>
   			</div>
   		</div>
   		</div>
