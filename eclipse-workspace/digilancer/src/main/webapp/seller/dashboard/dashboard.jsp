...
<body>
   ...
   <div class="dashboard container-fluid pr-5 pl-5 mb-5">
   		<div class="createproject mt-3 mr-3 d-flex justify-content-end">
   			<a href="http://localhost:8080/digilancer/seller/projects.jsp"> <i class="fa fa-search pr-1" aria-hidden="true"></i> Search Project  </a>
   		</div>
   		...
   		<div class="ml-3 mr-3 managefreelancers mt-4">
	   			<form  class="w-100 d-flex justify-content-between" action="" method="post">
		   			<input name="action" value="Saved Projects" type="submit">
		   			<input name="action" value="Applied Projects" type="submit">
	   			</form>
	   		</div>
	   		<!-- Releated Freelancers -->
	   		<%
	   		if(request.getParameter("action") != null && request.getParameter("action").equals("Saved Projects")){
	   		%>
	   			<%@ include file="savedprojects.jsp" %>
	   		<% }  else if(request.getParameter("action") != null && request.getParameter("action").equals("Applied Projects")){ %>
	   			<%@ include file="appliedprojects.jsp" %>
	   		<% } else{ %>
   		<%
		...
		<% } else if(request.getParameter("action1") != null && request.getParameter("action1").equals("managereviews")){ %>
   		...
   		<p class="ml-3 mr-3 mt-5 text-light">What to do next?</p>
	   		<div style="margin-bottom:34vh" class="ml-3 mr-3 whattodo d-flex justify-content-around align-items-center mt-3">
	   			<div class="steps d-flex align-items-center">
	   				<div class="number-border mr-2">1</div>
	   				<div class="content">View Projects</div>
	   			</div>
	   			...
	   			<div class="steps d-flex align-items-center justify-content-around">
	   				...
	   				<div class="content">Apply on Projects</div>
	   			</div>
	   			<img style="width:150px; height:20px" class="mr-4 ml-4" src="images/arrow.png"/>
	   			<div class="steps d-flex align-items-center">
	   				<div class="number-border mr-2">3</div>
	   				<div class="content">Chat with client</div>
	   			</div>
	   		</div>
   		<% } %>
   		<% } %>
